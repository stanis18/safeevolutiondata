...
pub fn parse_implementation(impl_path : &Path) -> Result<Implementation,String> {
    ...
    let file_name: Vec<&str> = impl_path.file_name().unwrap().to_str().unwrap().split("_").collect();

    let file = File::open(Path::new(&impl_path)).map_err( |_| {format!("The ast tree for the {} file could not be created.", file_name[0])})?;
    let reader = BufReader::new(file);
