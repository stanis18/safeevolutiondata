...
use crate::checker::{get_function_implementation,get_variables_implementation};
use crate::parser::{FunctionImplementation};

pub fn config_proxy (imp: &Implementation, impl_path: &Path){
    let mut file_implementation =
        ...
        data_implementation = data_implementation
                            .replace("/*constructor initialization */", &get_constructor_body(&imp, &impl_path).unwrap())
                            .replace("/* constructor parameters */", &get_constructor_paramts(&imp).unwrap())
                            .replace("/* variables */", &get_variables_proxy(&imp).unwrap())
                            .replace("/* functions */", &get_functions_proxy(&imp).unwrap());
