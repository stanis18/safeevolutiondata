...
// pub async fn deploy_contract_t(spec_id:&String, author_account:&Address, name_imp_contract: &String, 
...
//     // Not deploy registry
//     let registry_address = deploy_contract_blockchain(&author_account, &vec![].to_vec(), path_registry_bin, path_registry_abi).await?;
...
//         path_contract_bin.as_path(), path_contract_abi.as_path()).await?;
...
//      // Not Proxy
//     let proxy_address = deploy_contract_blockchain(&author_account, &vector, path_proxy_bin, path_proxy_abi).await?;
...
// pub fn get_assigned_variable(typ:String, value:String) -> Result<AssignedVariable, String> {
...
// pub async fn upgrade_contract_t(spec_id: &String, author_account: Address, registry_address: Address, proxy_address: Address, 
//     path_contract: &Path) -> Result <(), String> {
...
//     let path_contract_abi = Path::new("contracts/input").join(&contract_name_abi);
//     let path_contract_bin = Path::new("contracts/input").join(&contract_name_bin);
//     let contract_address = deploy_contract_blockchain(&author_account, &vec![].to_vec(), path_contract_bin.as_path(), path_contract_abi.as_path()).await?;
